...
if [ ! -f .env ]; then
    if [ "$PROFILE" = "split-local" ]; then
        echo "Error: .env file is required for split-local profile"
        echo ""
        echo "Create it from the template:"
        echo "  cp .env.example-local .env"
        echo "  # Then edit .env with your cloud MQTT settings"
        exit 1
    fi
...
# Validate required variables (only for profiles that need them)
if [ "$PROFILE" != "split-local" ]; then
    # InfluxDB and Grafana only run in local and split-cloud profiles
...
# Configure Grafana datasource with correct token (only for profiles with Grafana)
if [ "$PROFILE" != "split-local" ]; then
...
# Create init-buckets script if not exists (only for profiles with InfluxDB)
if [ "$PROFILE" != "split-local" ] && [ ! -f influxdb/init-buckets.sh ]; then
...
if [ "$PROFILE" = "split-local" ]; then
    docker compose -f docker-compose.split-local.yml pull
else
...
if [ "$PROFILE" = "split-local" ]; then
    docker compose -f docker-compose.split-local.yml up -d
else
...
echo "=== Service Status ==="
if [ "$PROFILE" = "split-local" ]; then
    docker compose -f docker-compose.split-local.yml ps
else
